...
from losses import (amplitude_mask, background_loss, contour_area_ratio_loss,
...
def main(args):
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        if iteration == 1 or iteration % args.log_every == 0 or iteration == args.iterations:
            ...
            print("[{}/{}] total={:.3e} amp={:.3e} projection_amp={:.3e} hist={:.3e} bg={:.3e} area={:.3e} "
                  "psnr={:.2f} ssim={:.3f} iou={:.3f} elapsed={} eta={}".format(
                      iteration, args.iterations, values[1], values[2], values[6], values[3], values[4], values[5],
                      values[7], values[8], values[12], format_duration(elapsed), format_duration(eta)))
