...
集中在 `backend/config.py`：默认训练 3000 轮；轮廓参数为 `sigma=16.0`、阈值为基线校正后峰值的 `0.20`，以形成整个分子的连续概括外包络；等点数分箱的箱数 `HISTOGRAM_BINS = 256`（取值依据见 `docs/histogram_quantile_research.md` 第七节）。离线指标及最终对比图会先消除平移和镜像歧义，再与原图比较。
